...
function envioTransacaoEther(parametro) {
  ...
  instance.funcao.sendTransaction(parametro, txn, (error, result) => {
    console.info(result);
    console.info(error);
  });
}
...
function consultarSaldo(jogoInstance) {
  instance.consultaSaldo((error, result) => {
    if (error) {
      console.info(error);
    } else {
...
$("#btnSendValidation").click(() => {
  ...
  personInstance.sendValidationIdentity.sendTransaction(_addressPerson, _valid, txn, (error, result) => {
    console.info(result);
    console.info(error);
  });
});