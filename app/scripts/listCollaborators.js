...
async function getListPerson() {
    ...
    board = Trello.get('/boards/59d632142cf55c63678af1a6?actions=none&boardStars=none&cards=none&card_pluginData=false&checklists=none&customFields=false&fields=name&lists=none&members=none&memberships=none&membersInvited=none&membersInvited_fields=all&pluginData=false&organization=false&organization_pluginData=false&myPrefs=false&tags=false&key=d25c1cb09fa2ee032c93a7c4128baaee', function(data){
        $("#nameProject").text(data.name);
        console.info(data);
    });

    members = await Trello.get('/boards/59d632142cf55c63678af1a6/memberships/?orgMemberType=true&member=true&member_fields=fullName,username&key=d25c1cb09fa2ee032c93a7c4128baaee&fields=id,name,idMembers,shortUrl');
    pointsTotal = 0;
    cardsTotal = 0; 
    await asyncForEach(members, async (membro) => {
        ...
        await asyncForEach(listCards, async (cards) => {
            if (cards.idBoard == "59d632142cf55c63678af1a6") {
